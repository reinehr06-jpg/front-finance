...
export default function EditarContaPage({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
  ...
  // Estados do Formulário (Simulando dados carregados)
  const [nome, setNome] = useState("Conta Itaú Principal");
  const [tipo, setTipo] = useState("corrente");
  const [status, setStatus] = useState("ativo");
  const [valor, setValor] = useState("28.650,00");
      ...
      <div className="flex-1 ml-[240px] flex flex-col h-screen overflow-hidden">
        ...
        <main className="p-[0_24px_24px_24px] flex-1 flex flex-col overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center gap-[14px] py-[16px]">
            <Link href={`/contas/${params.id}`} className="w-[32px] h-[32px] rounded-[8px] bg-white border border-[#E5E7EB] flex items-center justify-center hover:bg-[#F9FAFB] transition-colors shrink-0 shadow-sm text-[#6B7280]">
            ...
            <div className="flex flex-col justify-center">
              <h1 className="text-[24px] font-[700] text-[#1A1A2E] leading-tight">Editar Conta</h1>
              <p className="text-[13px] font-[500] text-[#6B7280] mt-0.5">Altere as informações da sua conta bancária.</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[16px] border border-[#E5E7EB] shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col p-[20px]">
            ...
            <div className="border-t border-[#F1F1F4] mt-[32px] pt-[24px] flex items-center justify-between">
              <div>
                <button type="button" className="px-[20px] py-[10px] bg-[#FEF2F2] hover:bg-[#FEE2E2] transition-colors text-[#EF4444] text-[12px] font-[700] uppercase tracking-wider rounded-[8px]">
                  EXCLUIR
                </button>
              </div>
              <div className="flex items-center gap-[12px]">
                <Link href="/menu/contas" className="px-[20px] py-[10px] bg-white border border-[#E5E7EB] hover:bg-[#F9FAFB] transition-colors text-[#4B5563] text-[12px] font-[700] rounded-[8px]">
                  CANCELAR
                </Link>
