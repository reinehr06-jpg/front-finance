...
export default function EditarCategoriaPage({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
  ...
  const [nome, setNome] = useState("Ofertas Especiais");
  const [status, setStatus] = useState("ativo");
  const [tipo, setTipo] = useState("receita");
  const [descricao, setDescricao] = useState("");

  const [nivel, setNivel] = useState("principal");
      ...
      <div className="flex-1 ml-[240px] flex flex-col h-screen overflow-hidden">
        ...
        <main className="p-[0_24px_24px_24px] flex-1 flex flex-col overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center gap-[14px] py-[16px]">
            <Link href="/menu/contabilidade/categorias" className="w-[32px] h-[32px] rounded-[8px] bg-white border border-[#E5E7EB] flex items-center justify-center hover:bg-[#F9FAFB] transition-colors shrink-0 shadow-sm text-[#6B7280]">
              <ChevronLeftIcon className="w-[18px] h-[18px]" />
            </Link>
            <div className="w-[44px] h-[44px] rounded-[10px] bg-[#FEF3C7] flex items-center justify-center shrink-0">
              <Tags className="w-[20px] h-[20px] text-[#F59E0B]" strokeWidth={2.2} />
            </div>
            <div className="flex flex-col justify-center">
              <h1 className="text-[24px] font-[700] text-[#1A1A2E] leading-tight">Editar Categoria</h1>
              <p className="text-[13px] font-[500] text-[#6B7280] mt-0.5">Edite as informações da categoria.</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[16px] border border-[#E5E7EB] shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col p-[20px]">
                ...
                <div className={`transition-all duration-300 ease-in-out bg-white ${openAccordion === "info" ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}>
                  <div className="p-[24px] flex flex-col gap-[24px]">
                    ...
                    <div className="grid grid-cols-12 gap-[24px]">
                      ...
                      <div className="col-span-6 flex flex-col gap-[8px]">
                        <label className="text-[12px] font-[600] text-[#374151]">
                          {t("Nível da Categoria")} <span className="text-[#EF4444] ml-0.5">*</span>
                        </label>
                        <CustomSelect
                          value={nivel}
                          onChange={setNivel}
                      ...
                      {nivel === "secundaria" && (
                        <div className="col-span-6 flex flex-col gap-[8px]">
                          <label className="text-[12px] font-[600] text-[#374151]">
                            {t("Categoria Principal")} <span className="text-[#EF4444] ml-0.5">*</span>
                          </label>
                          <CustomSelect
                            value={categoriaPai}
                            onChange={setCategoriaPai}
                            placeholder={t("Selecione a categoria principal...")}
                            searchable={true}
                            className="h-[38px]"
                            options={[{value: "dizimos", label: "Dízimos"}, {value: "energia", label: "Energia Elétrica"}]}
                          />
                        </div>
                      )}
                      ...
                      <div className="col-span-6 flex flex-col gap-[8px]">
                        ...
                        <textarea 
                          ...
                          className="h-[38px] w-full border border-[#E5E7EB] rounded-[8px] p-[10px_12px] text-[13px] text-[#111827] outline-none hover:border-[#D1D5DB] focus:border-[#6D28D9] focus:ring-1 focus:ring-[#6D28D9] transition-all bg-white resize-none" 
                        />
                      </div>
                    </div>
            ...
            <div className="border-t border-[#F1F1F4] mt-[32px] pt-[24px] flex items-center justify-end gap-[12px]">
              <Link href="/menu/contabilidade/categorias" className="px-[20px] py-[10px] bg-white border border-[#E5E7EB] hover:bg-[#F9FAFB] transition-colors text-[#4B5563] text-[12px] font-[700] rounded-[8px]">
                CANCELAR
              </Link>
